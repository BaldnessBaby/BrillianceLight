...
uint16_t BSP_W25QXX_TYPE=W25Q16;	//默认是W25Q32
#define DUMMY_BYTE 0xFF
uint8_t SPI_FLASH_ReadWriteByte(uint8_t _uc_byte)
{
    uint8_t uc_read,uc_send=_uc_byte;
    if (HAL_SPI_TransmitReceive(&hspi1,&uc_send,&uc_read,1,100)!=HAL_OK)
    uc_read = DUMMY_BYTE;
    return uc_read;
}
//4Kbytes为一个Sector
...
{ 
	...
	while(1) 
	{	
		BSP_W25QXX_Read(BSP_W25QXX_BUF,secpos*W25Q128FV_SUBSECTOR_SIZE,W25Q128FV_SUBSECTOR_SIZE);//读出整个扇区的内容
		for(i=0;i<secremain;i++)//校验数据
		{
			if(BSP_W25QXX_BUF[secoff+i]!=0XFF)break;//需要擦除  	  
		}
		if(i<secremain)//需要擦除
		{
			...
			BSP_W25QXX_Write_NoCheck(BSP_W25QXX_BUF,secpos*W25Q128FV_SUBSECTOR_SIZE,W25Q128FV_SUBSECTOR_SIZE);//写入整个扇区  

		}else BSP_W25QXX_Write_NoCheck(pBuffer,WriteAddr,secremain);//写已经擦除了的,直接写入扇区剩余区间. 	
...
{  
	//监视falsh擦除情况,测试用   
// 	printf("fe:%x\r\n",Dst_Addr);	  
 	Dst_Addr*=W25Q128FV_SUBSECTOR_SIZE;
    BSP_W25QXX_Write_Enable();                  	//SET WEL 	 
    BSP_W25QXX_Wait_Busy();   
  	W25Qx_Enable();                            	//使能器件   
...
{   
	while((BSP_W25QXX_ReadSR()&0x01)==0x01);  		// 等待BUSY位清空
}  
//进入掉电模式
